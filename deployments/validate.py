...
def validate_root_endpoint(base_url: str, timeout: float, verbose: bool) -> bool:
    ...
    try:
        ...
        if missing_fields:
            print_status(
                f"Root endpoint missing required fields: {missing_fields}",
                "error"
            )
            return False
...
def validate_health_endpoint(base_url: str, timeout: float, verbose: bool) -> bool:
    ...
    try:
        ...
        if missing_fields:
            print_status(
                f"Health endpoint missing required fields: {missing_fields}",
                "error"
            )
            return False
...
def wait_for_service(base_url: str, timeout: float, max_attempts: int = 30) -> bool:
    ...
    print_status(
        f"Service did not become available after {max_attempts} attempts",
        "error"
    )
    return False
